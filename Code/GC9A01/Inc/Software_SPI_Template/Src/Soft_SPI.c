...
static void mosi_low(Soft_SPI* spi_struct)
{
...
static void mosi_high(Soft_SPI* spi_struct)
{
...
static void sck_low(Soft_SPI* spi_struct)
{
  spi_struct->port->BSRR = (1 << (spi_struct->sck + BSRR_L_TO_H_OFFSET));
  timer_delay_us(spi_struct->clock_cycle_us);
}

// Sets the sck line high and waits for half a clock cycle
static void sck_high(Soft_SPI* spi_struct)
{
...
static void cs_low(Soft_SPI* spi_struct)
{
...
static void cs_high(Soft_SPI* spi_struct)
{
